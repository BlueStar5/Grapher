class Camera {
  ...
  drawLines(ctx: CanvasRenderingContext2D, lines, gridColors?) {
  ...
  ;
  drawVectors(ctx: CanvasRenderingContext2D, vectors) {
    vectors.forEach(v => {
      this.drawVector(ctx, v);
    });
  }
  ;
  drawVector(ctx: CanvasRenderingContext2D, v: Vector) {
